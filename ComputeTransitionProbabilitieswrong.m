...
Transition_probabilities_matrix = ones(K,K,5);
...
for i = 1 : K
    ...
    function wind_shooters(Transition_probabilities_matrix,map,stateSpace,i,row_j,col_j,shooters)
        
        for m = row_j-1 : row_j+1
                
                for n = col_j-1 : col_j+1
                    
                    %identifico la croce, compreso j
                    
                    is_neighbour_or_j = (abs(row_j-m) == 0 && abs(col_j-n) == 1) || (abs(row_j-m) == 1 && abs(col_j-n) == 0) || (abs(row_j-m) == 0 && abs(col_j-n) == 0);
                  
                    if(is_neighbour_or_j)
...
for i = 1 : K  %extracting two indexes
    ...
    for j = 1 : K
        ...
        if((abs(row_i - row_j) <= 1 )&& (abs(col_i - col_j) <= 1)) %checking if j is a neighbour of i
        ...
        for u = 1 : 5
            ...
            if(stateSpace(i,3) == stateSpace(j,3))%only transitions where the 'package state' is consistent except for the pickup station
                
                if(u == EAST)
                    
                    if(col_j == col_i + 1 && row_j == row_i)  %regular movement
                        
                        if(Transition_probabilities_matrix(i,j,u) == 0)
                    ...
                    wind_shooters(Transition_probabilities_matrix,map,stateSpace,i,row_j,col_j,shooters)
            ...
            %still need to count for pick up station
...
disp(stateSpace(9,1));
disp(stateSpace(9,2));
disp(stateSpace(13,1));
disp(stateSpace(13,2));
disp(P(9,11,1));
disp(P(9,11,2));
disp(P(9,11,3));
disp(P(9,11,4));
