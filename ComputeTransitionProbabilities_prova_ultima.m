function P = ComputeTransitionProbabilities_(stateSpace, map)
...
Crashing_probabilities = cat(1,Crashing_probabilities,a);
Crashing_probabilities = cat(2,Crashing_probabilities,b);
...
for i = 1 : K  % prendo il primo stato
    ...
    for j = 1 : K   % prendo il secondo stato
        ...
        pack_j = stateSpace(j,3);
         ...
         if((m_j == m_i + 1 && n_j == n_i) || (m_j == m_j - 1 && n_j == n_j) || (m_j == m_i && n_j == n_i + 1) || (m_j == m_i && n_j == n_i - 1) || (m_j == m_i && n_j == n_i))   
...
%                                     Transition_probabilities_matrix(i,base,u) = Transition_probabilities_matrix(i,base,u) + 0.25 * P_WIND * Crashing_probabilities(stateSpace(final_state,1),stateSpace(final_state,2));
                                    ...
                                    if(m >= 1 && m <= size(map,1) && n >= 1 && m <=size(map,2))
